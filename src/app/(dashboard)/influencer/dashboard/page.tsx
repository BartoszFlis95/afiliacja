...
export default async function InfluencerDashboardPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div
        data-surface="dark"
        className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 p-6 text-white md:p-8"
      >
          ...
          <CardContent className={topLinks.length === 0 ? "p-0" : "space-y-3 px-6 pb-6"}>
            {topLinks.length === 0 ? (
              ...
              topLinks.map((link) => (
                <div
                  key={link.id}
                  className="flex flex-col gap-3 rounded-xl border border-border/60 p-4 transition-all duration-200 hover:border-border hover:shadow-sm sm:flex-row sm:items-center sm:justify-between"
                >
      ...
      {availableProducts.length > 0 && (
        <div>
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-base font-semibold text-foreground">Dostępne produkty</h2>
            <Link
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {availableProducts.map((product) => (
              <Link
                key={product.id}
                href="/influencer/products"
                className="group overflow-hidden rounded-xl border border-border/60 bg-card shadow-sm transition-all duration-200 hover:-translate-y-[1px] hover:shadow-md"
              >
                <div className="relative h-24 w-full bg-muted/50">
                  {product.imageUrl ? (
                    <Image

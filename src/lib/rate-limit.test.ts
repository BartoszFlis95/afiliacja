...
describe("checkRateLimit", () => {
  ...
  it("przepuszcza dokładnie 3 próby i blokuje czwartą", async () => {
    const k = klucz("limit");
    expect((await checkRateLimit(k)).allowed).toBe(true);
    expect((await checkRateLimit(k)).allowed).toBe(true);
    expect((await checkRateLimit(k)).allowed).toBe(true);
    expect((await checkRateLimit(k)).allowed).toBe(false);
  });

  it("po przekroczeniu limitu podaje, za ile można ponowić", async () => {
    const k = klucz("retry");
    for (let i = 0; i < 3; i++) await checkRateLimit(k);
    const wynik = await checkRateLimit(k);
  ...
  it("kolejne zablokowane próby NIE przedłużają okna", async () => {
    ...
    for (let i = 0; i < 3; i++) await checkRateLimit(k);
    const pierwsza = await checkRateLimit(k);
    const druga = await checkRateLimit(k);
    if (!pierwsza.allowed && !druga.allowed) {
      expect(druga.retryAfterMs).toBeLessThanOrEqual(pierwsza.retryAfterMs);
    }
  });

  it("zwalnia limit po upływie godziny", async () => {
    vi.useFakeTimers();
    const k = klucz("okno");
    for (let i = 0; i < 3; i++) await checkRateLimit(k);
    expect((await checkRateLimit(k)).allowed).toBe(false);
  ...
  it("liczy każdy klucz osobno — jeden adres nie blokuje innych", async () => {
    const a = klucz("ip-a");
    const b = klucz("ip-b");
  ...
  it("rozdziela akcje: wyczerpany reset hasła nie blokuje wysyłki weryfikacji", async () => {
    const ip = Math.random().toString(36).slice(2);
    for (let i = 0; i < 3; i++) await checkRateLimit(`forgot-password:${ip}`);
    expect((await checkRateLimit(`forgot-password:${ip}`)).allowed).toBe(false);
    expect((await checkRateLimit(`resend-verification:${ip}`)).allowed).toBe(true);
  });
   ...
   */
  it("wytrzymuje zalanie wieloma adresami i nadal egzekwuje limit", async () => {
    const t0 = performance.now();
    for (let i = 0; i < 30_000; i++) await checkRateLimit(`zalew:${i}`);
    ...
    expect((await checkRateLimit(swiezy)).allowed).toBe(true);
    expect((await checkRateLimit(swiezy)).allowed).toBe(true);
    expect((await checkRateLimit(swiezy)).allowed).toBe(true);
    expect((await checkRateLimit(swiezy)).allowed).toBe(false);
  });

  it("przyjmuje własne progi, nie tylko domyślne", async () => {
    const k = klucz("wlasny-prog");
    for (let i = 0; i < 10; i++) {
      expect((await checkRateLimit(k, 10, 60_000)).allowed).toBe(true);
    }
    expect((await checkRateLimit(k, 10, 60_000)).allowed).toBe(false);
  });
   ...
   */
  it("ma progi dobrane pod charakter akcji", async () => {
